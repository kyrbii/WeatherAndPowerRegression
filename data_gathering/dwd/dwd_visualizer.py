...
def map_data_file_2_colors(data_file, header_rows, leading_columns):
    red_pixels = [[],[]]
    green_pixels = [[],[]]
    blue_pixels = [[],[]]
        ...
        for current_row in csv_reader:
            ...
            if rowidx >= header_rows:
                ...
                for current_cell in current_row:
                    x_pos = colidx - leading_columns # compensage date, hour and station
                    if (colidx >= leading_columns):
                        # map
                        value = float(current_cell)
                        # print(f"{x_pos}/{y_pos} -> {value}")
                        if value == -999.0:
    ...
    return red_pixels, green_pixels, blue_pixels, (colidx - leading_columns), (rowidx - header_rows)
...
if __name__ == '__main__':
    ...
    mapped_red, mapped_green, mapped_blue, max_x, max_y = map_data_file_2_colors(data_file, header_rows, leading_columns)
